...
select restaurant_id,restaurant_name,order_id,order_date ,dayname(order_date) as order_day from restaurant as r 
join orders as o
on rest_id=restaurant_id
group by 1,2,3,4 order by 1);
select restaurant_id,restaurant_name,order_id,order_date ,order_day,count(restaurant_id) as frequency ,
rank() over(partition by restaurant_id order by count(restaurant_id)) from frequency
